 ...
 * ulordj for a specific network. You can get an instance of this class through calling {@link #get()}.</p>
 *
...
public class Context {
    ...
    public Context(NetworkParameters params) {
        log.trace("Creating ulordj {} context.", ULORDJ_VERSION);
        this.params = params;
     ...
     * independently use ulordj (or possibly alt coin forks of it) can operate correctly.
     *
    ...
    public static Context get() {
        ...
        if (tls == null) {
            if (isStrictMode) {
                log.error("Thread is missing a ulordj context.");
                log.error("You should use Context.propagate() or a ContextPropagatingThreadFactory.");
                throw new IllegalStateException("missing context");
            }
            if (lastConstructed == null)
                throw new IllegalStateException("You must construct a Context object before using ulordj!");
            slot.set(lastConstructed);
            log.error("Performing thread fixup: you are accessing ulordj via a thread that has not had any context set on it.");
    ...
    /**
     * Require that new threads use {@link #propagate(Context)} or {@link co.usc.ulordj.utils.ContextPropagatingThreadFactory},
